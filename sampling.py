...
class AdaptivePredictor(Predictor):
    ...
    def update_fn(self, rng, x, t, h, x_prev):
        ...
        h_max = jnp.maximum(t_ - self.eps,
                            0)  # max step-size must be the distance to the end (we use maximum between that and zero in case of a tiny but negative value: -1e-10)
        E_pow = jnp.where(h_ == 0, h_, jnp.power(E_scaled_norm,
...
class DDIMPredictor(Predictor):
    ...
    def vpsde_update_fn(self, rng, x, t, h, x_prev=None):
        ...
        x = batch_mul(jnp.sqrt(alpha_next),
                      batch_mul(x - batch_mul(jnp.sqrt(1. - alpha), score), 1. / jnp.sqrt(alpha))) + batch_mul(
            jnp.sqrt(1 - alpha_next), score)
        return x, x
...
def shared_predictor_update_fn(rng, state, x, t, h=None, x_prev=None, sde=None, shape=None, model=None, predictor=None,
                               probability_flow=None, continuous=None,
                               eps=1e-3, abstol=1e-2, reltol=1e-2, error_use_prev=True, norm="L2_scaled", safety=.9,
                               extrapolation=False, sde_improved_euler=False, exp=0.9):
    ...
    if predictor is None:
        ...
        predictor_obj = predictor(sde, score_fn, shape, probability_flow, eps=eps, abstol=abstol, reltol=reltol,
                                  error_use_prev=error_use_prev, norm=norm, safety=safety, extrapolation=extrapolation,
                                  sde_improved_euler=sde_improved_euler, exp=exp)
...
def get_pc_sampler(sde, model, shape, predictor, predictor_name, corrector, corrector_name, inverse_scaler, snr,
    ...
    def pc_sampler(rng, state):
        ...
        h = timesteps - jnp.append(timesteps, 0)[
        ...
        if denoise:  # Tweedie formula
            ...
            score_fn = mutils.get_score_fn(sde, model, state.params_ema, state.model_state, train=False,
                                           continuous=continuous)
